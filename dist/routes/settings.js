...
router.get('/settings/secrets', projects_1.requireAdminAuth, async (_req, res) => {
    try {
        const results = await Promise.all(SECRET_DEFINITIONS.map(async (def) => {
            try {
        ...
        (0, projects_1.log)('settings.secrets.list.error', { error: error.message });
...
router.put('/settings/secrets/:name', projects_1.requireAdminAuth, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            (0, projects_1.log)('settings.secret.created', { name });
        }
        ...
        (0, projects_1.log)('settings.secret.updated', { name, updatedBy: req.user?.email || 'unknown' });
        res.json({ ok: true, message: `Secret '${def.label}' updated successfully.` });
    }
    catch (error) {
        (0, projects_1.log)('settings.secret.update.error', { name, error: error.message });
