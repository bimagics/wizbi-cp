document.addEventListener('DOMContentLoaded', () => {
  ...
  const ICONS = {
    ...
    ORGS: `<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>`,
    USERS: `<svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M15 21a6 6 0 00-9-5.197M15 11a4 4 0 110-5.292"/></svg>`,
    ...
    GCP: `<svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12.0001 2.00015c-.4021 0-.7989.1125-1.1458.32508L3.1451 6.8249C2.4508 7.25143 2 8.02008 2 8.8471v6.3061c0 .827.4508 1.5957 1.1451 2.0222l7.7092 4.4997c.3469.2125.7437.3251 1.1458.3251s.7989-.1126 1.1458-.3251l7.7092-4.4997c.6944-.4265 1.1451-1.1952 1.1451-2.0222V8.8471c0-.82702-.4507-1.59567-1.1451-2.0222L13.1459 2.32523c-.3469-.21258-.7437-.32508-1.1458-.32508Zm0 1.86415L19.071 8.00015 12.0001 12.1361 4.9292 8.00015 12.0001 3.8643ZM11.0001 13.2323v6.6997L4.3541 15.8232l6.646  -2.5909Zm2 0 6.646 2.5909-6.646 6.6997v-9.2906Z"/></svg>`,
    DELETE: `<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>`,
    EDIT: `<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>`,
    ERROR: `<svg fill="none" viewBox="0 0 24 24" stroke="currentColor" style="width:18px;height:18px;color:var(--error-color);" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>`
  };
  ...
  firebaseAuth.onAuthStateChanged(async (user) => {
    ...
    if (user) {
      ...
      idToken = null; userProfile = null; showView('loginContainer');
    }
  });
  ...
  const setupDashboard = () => {
    ...
    navItems.forEach(item => {
      ...
      button.addEventListener('click', () => {
        switchTab(item.id);
        // נסגור תפריט במובייל אחרי ניווט
    ...
    loadOrgs();
    loadProjects();
    if (userProfile.roles?.superAdmin) loadUsers();

    const createOrgBtn = document.getElementById('btnShowCreateOrg');
    if (createOrgBtn) createOrgBtn.classList.toggle('hidden', !userProfile.roles?.superAdmin);
  ...
  const loadProjects = async () => {
    ...
    try {
      ...
      if (isProcessing && !projectsRefreshInterval) {
        ...
        clearInterval(projectsRefreshInterval); projectsRefreshInterval = null;
      }
    } finally {
      spinner.style.display = 'none';
    }
  ...
  // ----- Renderers (with data-labels for mobile) -----
  const renderProjectsTable = (projects) => {
    const tbody = document.getElementById('projectsTable').querySelector('tbody');
    if (projects.length === 0) {
      tbody.innerHTML = `<tr><td data-label="Info" colspan="7">No projects found.</td></tr>`;
      return;
    }

    tbody.innerHTML = projects.map(p => {
      const isFailed = ['failed', 'delete_failed'].includes(p.state);
      const showSpinner = ['starting', 'provisioning', 'deleting'].includes(p.state);
      return `
        <tr>
          <td data-label="Display Name">${p.displayName}</td>
          <td data-label="Project ID">${p.id}</td>
          <td data-label="Org ID">${p.orgId}</td>
          <td data-label="State" class="state-cell">
            <div class="state-cell-text ${p.state}">${p.state || 'N/A'}</div>
            <span class="slot spinner-slot">${showSpinner ? '<div class="spinner spinner-inline" aria-label="Processing"></div>' : ''}</span>
            <span class="slot error-slot">${isFailed ? ICONS.ERROR : ''}</span>
          </td>
          <td data-label="Links" class="links-cell">
            ${p.gcpProjectId ? `<a href="https://console.cloud.google.com/home/dashboard?project=${p.gcpProjectId}" target="_blank" class="icon-button" title="Open in GCP Console" aria-label="Open in GCP Console">${ICONS.GCP}</a>` : ''}
            ${p.githubRepoUrl ? `<a href="${p.githubRepoUrl}" target="_blank" class="icon-button" title="Open in GitHub" aria-label="Open in GitHub">${ICONS.GITHUB}</a>` : ''}
          </td>
          <td data-label="Created">${new Date(p.createdAt).toLocaleDateString()}</td>
          <td data-label="Actions" class="actions-cell">
            ${userProfile.roles?.superAdmin ? `<button class="icon-button delete" data-type="project" data-id="${p.id}" data-name="${p.displayName}" title="Delete Project" aria-label="Delete Project">${ICONS.DELETE}</button>` : ''}
          </td>
        </tr>
      `;
    }).join('');
  ...
  const renderOrgsTable = (orgs) => {
    ...
    if (orgs.length === 0) {
      tbody.innerHTML = `<tr><td data-label="Info" colspan="5">No organizations found.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = orgs.map(org => `
      <tr>
        ...
        <td data-label="Actions" class="actions-cell">
          ${userProfile.roles?.superAdmin ? `<button class="icon-button delete" data-type="org" data-id="${org.id}" data-name="${org.name}" title="Delete Organization" aria-label="Delete Organization">${ICONS.DELETE}</button>`: ''}
        </td>
      </tr>
    `).join('');
  };
  ...
  const renderUsersTable = (users) => {
    ...
    if (users.length === 0) {
      tbody.innerHTML = `<tr><td data-label="Info" colspan="4">No users found.</td></tr>`;
      return;
    }
    ...
    tbody.innerHTML = users.map(user => {
      ...
      return `
        <tr>
          ...
          <td data-label="Organizations">${orgNames || 'N/A'}</td>
          <td data-label="Actions" class="actions-cell">
            <button class="icon-button" data-action="edit-user" data-uid="${user.uid}" title="Edit User Roles" aria-label="Edit User Roles">${ICONS.EDIT}</button>
          </td>
        </tr>
      `;
    }).join('');
  ...
  const updateOrgDropdown = (orgs) => {
    ...
    select.innerHTML = '<option value="" disabled selected>Select an Organization</option>' +
      orgs.map(org => `<option value="${org.id}" data-name="${org.name}">${org.name}</option>`).join('');
  ...
  DOM.userEditForm.addEventListener('submit', async (e) => {
    ...
    const updatedRoles = { superAdmin: isSuperAdmin, orgAdmin: orgAdminOf };
