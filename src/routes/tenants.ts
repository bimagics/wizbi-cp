...
export async function requireUser(req: Request, res: Response, next: Function) {
  try {
    ...
    let token = '', via: 'x-header'|'auth-bearer'|'none' = 'none';
    if (hdrX) { token = hdrX; via = 'x-header'; }
    else if (hdrAuth.startsWith('Bearer ')) { token = hdrAuth.slice(7); via = 'auth-bearer'; }
    if (!token) { log('auth.missing_token',{path:req.path}); return res.status(401).json({error:'missing token'}); }
...
function requireAdmin(req: Request, res: Response, next: Function) {
  const email = (((req as any).user as any)?.email||'').toLowerCase();
...
// ---- Google auth client
async function gauth() { return await google.auth.getClient({ scopes: ['https://www.googleapis.com/auth/cloud-platform'] }); }

// ---- Helpers (פרוביז׳ן) – אין שינוי לוגיקה, רק לוגים
async function waitProjectActive(crm:any, projectId:string, timeoutMs=180000){
  const deadline=Date.now()+timeoutMs, name=`projects/${projectId}`;
  for(;;){
...
async function linkBilling(projectId:string, billingAccount:string){
  const auth=await gauth(); const billing=google.cloudbilling({version:'v1',auth});
...
async function enableApis(projectId:string, services:string[]){
  const auth=await gauth(); const su=google.serviceusage({version:'v1',auth}); const parent=`projects/${projectId}`;
  for(let i=0;i<services.length;i+=20){ const list=services.slice(i,i+20);
...
async function createArtifactRegistry(projectId:string, region:string){
  const auth=await gauth(); const ar=google.artifactregistry({version:'v1',auth}); const parent=`projects/${projectId}/locations/${region}`;
  try{ log('provision.ar.create.start',{projectId,region});
  ...
  }catch(e:any){ const s=String(e); if(!s.includes('ALREADY_EXISTS')){ log('provision.ar.create.error',{projectId,error:s}); throw e; }
...
async function createBigQueryDatasets(projectId:string, datasets:string[], location='EU'){
  ...
  for(const ds of datasets){
    try{ log('provision.bq.dataset.create.start',{projectId,dataset:ds,location});
      await bq.createDataset(ds,{location});
      log('provision.bq.dataset.create.done',{projectId,dataset:ds});
    }catch(e:any){ const s=String(e); if(!s.includes('Already Exists')){ log('provision.bq.dataset.create.error',{projectId,dataset:ds,error:s}); throw e; }
...
async function createFirestoreDatabase(projectId:string, locationId='europe-west1'){
  const auth=await gauth(); const fs=google.firestore('v1');
  try{ log('provision.firestore.create.start',{projectId,locationId});
  ...
  }catch(e:any){ const s=String(e); if(!(s.includes('ALREADY_EXISTS')||s.includes('already exists'))){ log('provision.firestore.create.error',{projectId,error:s}); throw e; }
...
async function provisionTenant(tenantId:string, displayName:string, env:'qa'|'prod'){
  ...
  const docRef=TENANTS.doc(`${tenantId}-${env}`); const startedAt=new Date().toISOString();
...
router.post('/tenants/provision', requireUser, requireAdmin, async (req,res)=>{
  ...
  }catch(e:any){ log('tenants.provision.error',{error:String(e)}); res.status(500).json({ ok:false, error:String(e) }); }
});
