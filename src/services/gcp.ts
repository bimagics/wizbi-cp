...
async function addFirebase(firebase: firebase_v1beta1.Firebase, projectId: string) {
    ...
    try {
        ...
        if (error.code === 409) {
            ...
            // It might fail if App Engine is not ready, add a retry.
...
async function setupWif(iam: iam_v1.Iam, projectId: string, projectNumber: string, saEmail: string): Promise<string> {
    ...
    const poolPath = `projects/${projectId}/locations/global/workloadIdentityPools/${poolId}`;
...
// Keep existing functions for folder management if they are still needed for orgs
export { createGcpFolderForOrg, deleteGcpFolder, deleteGcpProject } from './gcp_legacy';
